...
class Processor():
    ...
    def print_log_cusmtom(self, msg):
        ...
        if self.arg.print_log:
            with open(f'{self.arg.work_dir}/{platform.node()}_summary1_log.txt', 'a') as f:
                print(msg, file=f)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            # print(f'train(), batch: {batch_idx}, input data, 0t, 2j, 0m', data[:,:,0,2,0]) # rand check
            # print(f'train(), batch: {batch_idx}, input data, 0t, 2j, 0m', data.size()) # rand check
        ...
        if save_model:
            ...
            torch.save(weights,
                       self.arg.model_saved_name + '-' + str(epoch).zfill(2) + '-' + str(int(self.global_step)).zfill(
                           5) + '.pt')
